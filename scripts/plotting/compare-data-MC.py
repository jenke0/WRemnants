...
parser.add_argument(
    "-s", "--study", type=str, help="the name of study for the combine files"
)
parser.add_argument(
    "-o",
    "--outdir",
...
if args.channels == "all":
    ...
    chns = args.channels.split(",")
if args.observable == "unrolled":
    action = lambda x: hh.unrolledHist(x)
else:
    action = lambda x: x.project(args.observable)
...
for chn in chns:
    for pdf in args.pdf:
        for pseudo in filter(lambda x: x != pdf, args.pdf):
            ...
            stacked_procs = list(
                filter(
                    lambda x: x not in [f"pdf{pseudo.upper()}_sum", "Data"],
                    hist_info.keys(),
                )
            )
            ...
            fig = plot_tools.makeStackPlotWithRatio(
                hist_info,
                stacked_procs,
                unstacked=f"pdf{pseudo.upper()}_sum",
                action=action,
                nlegcols=2,
                grid=True,
                rrange=args.rrange,
