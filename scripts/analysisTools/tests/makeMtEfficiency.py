...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        nargs=1,
        help="Input file with histograms for deepMet",
    )
    parser.add_argument(
        "hdf5filePFMet", type=str, nargs=1, help="Input file with histograms for PFMet"
    )
    parser.add_argument("outputfolder", type=str, nargs=1)

    parser.add_argument(
        "--mt", type=str, nargs="+", default=["mt_MET"], help="mt histograms to use"
    )
    parser.add_argument(
        "--mtleg", type=str, nargs="+", default=["PFMET"], help="mt name for legend"
    )
    parser.add_argument(
        "--signal",
        type=str,
        default="Wmunu_plus",
        help="Process to be used for signal efficiency",
    )
    parser.add_argument(
        "--postfix", type=str, default=None, help="Postfix for output folder"
    )
    parser.add_argument(
        "--invertmt",
        default=False,
        action="store_true",
        help="Invert mt cut to derive efficiency (default is mT>threshold)",
    )
...
def getHistograms(inputfile):
        ...
        hnarf = hnarf[
            {
                ...
                "pt": s[
                    0 : hist.len : hist.sum
                ],  # make sure to remove overflow here, we often don't cut on pt
                "hasJets": s[:: hist.sum],
            }
        ]
    ...
    for mt in args.mt:
        ...
        hqcd[mt].Add(
            nomihists[mt]["background"], -1.0
        )  # "background includes everything but data
        cropNegativeContent(
            hqcd[mt], cropValue=0.0
        )  # QCD can get negative from data-MC, set negative bins to 0 not to bias efficiency
    ...
    err = ctypes.c_double(
        0.0
    )  # or ROOT.Double(0.0) (deprecated in newer ROOT versions), just to pass a reference and use TH1::IntegralAndError()
        ...
        for ip, proc in enumerate(cutEff[mt].keys()):
            ...
            for mtcutbin in mtcutbins:
                if args.invertmt:
                    ...
                    eff = nomihists[mt][proc].IntegralAndError(
                        mtcutbin, nBinsTot + 1, err
                    )
                    eff /= nomihists[mt][proc].IntegralAndError(0, nBinsTot + 1, err)
                cutEff[mt][proc].SetPoint(
                    cutEff[mt][proc].GetN(),
                    nomihists[mt][proc].GetBinLowEdge(mtcutbin),
                    eff,
                )
            ...
            cutEff[mt][proc].SetMarkerStyle(
                (20 + imt) if len(args.mt) > 2 else (20 + imt + 4 * imt)
            )  # for two cases it should make full or open marker
    ...
    drawGraphCMS(
        grList,
        "Transverse mass threshold (GeV)",
        f"Efficiency for m_{{T}} {signCut} threshold::0.0,1.5",
        "mtCutEfficiency",
        outdir,
        grLeg,
        legendCoords="0.15,0.7,0.9,0.94;2",
        passCanvas=canvas1D,
        graphDrawStyle="pl",
        legEntryStyle="PL",
    ...
    for den in [args.signal, "background"]:
        ...
        for imt, mt in enumerate(args.mt):
            ...
            for mtcutbin in mtcutbins:
                if args.invertmt:
                    ...
                    sob = nomihists[mt]["data_fakes"].IntegralAndError(
                        mtcutbin, nBinsTot + 1, err
                    )
                    sob /= nomihists[mt][den].IntegralAndError(
                        mtcutbin, nBinsTot + 1, err
                    )
                grSoverB[mt].SetPoint(
                    grSoverB[mt].GetN(),
                    nomihists[mt]["background"].GetBinLowEdge(mtcutbin),
                    sob,
                )
                maxy = max(maxy, sob)
                miny = min(miny, sob)
        ...
        drawGraphCMS(
            [grSoverB[imt] for imt in grSoverB.keys()],
            "Transverse mass threshold (GeV)",
            f"QCD/{denLabel} yields for m_{{T}} {signCut} threshold::{miny},{maxy}",
            f"QCDover{denLabel}_yieldRatio",
            outdir,
            grLeg2,
            legendCoords="0.6,0.48,0.9,0.64;1",
            passCanvas=canvas1D,
            graphDrawStyle="pl",
            legEntryStyle="PL",
            useOriginalGraphStyle=True,
