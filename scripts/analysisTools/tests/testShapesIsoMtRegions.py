...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--useQCDMC",
        action="store_true",
        help="Use QCD MC instead of Fakes for MC stack",
    )
    parser.add_argument(
        "--noFake", action="store_true", help="Do not use Fakes for MC stack"
    )
    ...
    for isoMtID in args.isoMtRegion:
        ...
        for d in datasets:
            ...
            for charge in charges:
                ...
                if d == "Fake":
                    ...
                    histForFRF[charge][isoMtID] = copy.deepcopy(
                        h.Clone(f"{d}_{charge}_{regKey}")
                    )
        ...
        if "Data" in datasets:
            ...
            ratioRange = (
                [0.92, 1.08]
                if isoMtID == 3
                else [0.5, 1.5] if isoMtID == 2 else [0.0, 2.0]
            )
            for c in charges:
                hdata2D = hist2D[c][f"Data_{c}"]
                hmc2D = [
                    hist2D[c][key]
                    for key in hist2D[c].keys()
                    if key.split("_")[0]
                    not in ["Data", "Fake" if args.useQCDMC else "QCD"]
                ]
        ...
        for c in charges:
            if all(x in args.isoMtRegion for x in [0, 1]):
                ...
                drawSingleTH1(
                    hFRF_unrolled,
                    XlabelUnroll,
                    f"Fake rate factor ({c})",
                    f"{hFRF.GetName()}_unrolled",
                    outdir,
                    drawLineTopPanel=1.0,
                    drawLineLowerPanel="",
                    lowerPanelHeight=0.4,
                    labelRatioTmp="Rel. stat. unc.::0.5,1.5",
                    topMargin=0.06,
                    passCanvas=cwide,
                    ...
                    drawVertLines="{a},{b}".format(
                        a=hFRF.GetNbinsY(), b=hFRF.GetNbinsX()
                    ),
            ...
            for isoMtID in [2, 3]:
                if isoMtID in args.isoMtRegion:
                    ...
                    hFakeYields_unrolled = unroll2Dto1D(
                        hFakeYields, newname=f"{hFakeYields.GetName()}_unrolled"
                    )
                    drawSingleTH1(
                        hFakeYields_unrolled,
                        XlabelUnroll,
                        f"Events ({c})",
                        ...
                        lumi=16.8,
                        drawVertLines="{a},{b}".format(
                            a=hFakeYields.GetNbinsY(), b=hFakeYields.GetNbinsX()
                        ),
                        textForLines=ptBinRanges,
                        ytextOffsetFromTop=0.3,
                        textSize=0.04,
