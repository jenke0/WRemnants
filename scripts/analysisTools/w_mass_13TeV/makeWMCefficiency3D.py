...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="yieldsForWeffMC",
    )
    parser.add_argument(
        "--passMt",
        action="store_true",
        help="Measure efficiencies only for events passing mT, otherwise stay inclusive",
    )
    parser.add_argument(
        "-p",
        "--processes",
    ...
    parser.add_argument(
        ...
        default=-1,
        type=int,
        help="If positive, rebin yields versus uT by this number before deriving efficiencies in 3D",
    )
    ...
    for d in datasets:
        ...
        hiso_passTrigger = h[{"charge": s[:: hist.sum], "passTrigger": True}]
        hiso_failTrigger = h[{"charge": s[:: hist.sum], "passTrigger": False}]
        hiso_noTrigger = h[{"charge": s[:: hist.sum], "passTrigger": s[:: hist.sum]}]
        ...
        htriggerplus = h[{"charge": s[complex(0, 1)], "passIso": s[:: hist.sum]}]
        htriggerminus = h[{"charge": s[complex(0, -1)], "passIso": s[:: hist.sum]}]
        ...
        eff_iso_boost2D, eff_iso = getEtaPtEff(
            n_iso_pass,
            n_iso_tot,
            getRoot=True,
            rootName=f"{d}_MC_eff_iso",
            rootTitle="P(iso | trig)",
        )
        drawCorrelationPlot(
            eff_iso,
            xAxisName,
            yAxisName,
            f"MC isolation efficiency",
            f"{eff_iso.GetName()}",
        ...
        eff_isonotrig_boost2D, eff_isonotrig = getEtaPtEff(
            n_isonotrig_pass,
            n_isonotrig_tot,
            getRoot=True,
            rootName=f"{d}_MC_eff_isonotrig",
            rootTitle="P(iso | ID+IP)",
        )
        drawCorrelationPlot(
            eff_isonotrig,
            xAxisName,
        ...
        eff_isoantitrig_boost2D, eff_isoantitrig = getEtaPtEff(
            n_isoantitrig_pass,
            n_isoantitrig_tot,
            getRoot=True,
            rootName=f"{d}_MC_eff_isoantitrig",
            rootTitle="P(iso | ID+IP & fail trig)",
        )
        drawCorrelationPlot(
            eff_isoantitrig,
            xAxisName,
        ...
        eff_triggerplus_boost2D, eff_triggerplus = getEtaPtEff(
            n_triggerplus_pass,
            n_triggerplus_tot,
            getRoot=True,
            rootName=f"{d}_MC_eff_triggerplus",
            rootTitle="P(trig | ID+IP)",
        )
        drawCorrelationPlot(
            eff_triggerplus,
            xAxisName,
            yAxisName,
            f"MC trigger plus efficiency",
            f"{eff_triggerplus.GetName()}",
            plotLabel="ForceTitle",
            outdir=outdir,
            smoothPlot=False,
            drawProfileX=False,
        ...
        eff_triggerminus_boost2D, eff_triggerminus = getEtaPtEff(
            n_triggerminus_pass,
            n_triggerminus_tot,
            getRoot=True,
            rootName=f"{d}_MC_eff_triggerminus",
            rootTitle="P(trig | ID+IP)",
        )
        drawCorrelationPlot(
            eff_triggerminus,
            xAxisName,
            yAxisName,
            f"MC trigger minus efficiency",
            f"{eff_triggerminus.GetName()}",
            plotLabel="ForceTitle",
            outdir=outdir,
            smoothPlot=False,
            drawProfileX=False,
        ...
        eff_triggerplus_3D = getBoostEff(
            n_triggerplus_pass, n_triggerplus_tot, rebinUt=args.rebinUt
        )
        eff_triggerminus_3D = getBoostEff(
            n_triggerminus_pass, n_triggerminus_tot, rebinUt=args.rebinUt
        )
        ...
        hTestUt = hTestUt[
            {
                ...
                "pt": s[
                    0 : nPtBins : hist.sum
                ],  # would s[::hist.sum] sum overflow pt bins?
                "passTrigger": True,
                "passIso": True,
            }
        ]
        for charge in [-1, 1]:
            ...
            for ih, htmp in enumerate(allHists):
                ...
                hroot.Scale(
                    1.0 / hroot.Integral()
                )  # normalize to unit area to get shape
            ...
            drawNTH1(
                ...
                draw_both0_noLog1_onlyLog2=1,
                topMargin=0.05,
                labelRatioTmp="X / incl.::0.5,1.5",
                legendCoords="0.2,0.8,0.77,0.92;1",
                passCanvas=canvas1D,
                skipLumi=True,
                onlyLineColor=True,
