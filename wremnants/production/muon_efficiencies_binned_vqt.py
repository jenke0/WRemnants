...
narf.clingutils.Declare('#include "muon_efficiencies_binned_vqt.hpp"')

data_dir = common.data_dir


def make_muon_efficiency_helpers_binned_vqt(
    ...
    vector = ROOT.std.vector("TF1")()
    for i in range(0, 8):
        for j in range(0, 15):
            fitname = f"fa1_{j}_{i}"
            vector.push_back(finvqt.Get(fitname))

    helper = ROOT.wrem_vqt.muon_efficiency_binned_helper[
        str(is_w_like).lower(),
    ...
    helper2 = ROOT.wrem.muon_efficiency_binned_helper[
        str(is_w_like).lower(),
        type(sf_other_pyroot),
        type(sf_tracking_pyroot),
        type(sf_reco_pyroot),
    ...
    helper_syst = ROOT.wrem.muon_efficiency_binned_helper_syst[
        str(is_w_like).lower(),
        type(sf_other_pyroot),
        type(sf_tracking_pyroot),
        type(sf_reco_pyroot),
