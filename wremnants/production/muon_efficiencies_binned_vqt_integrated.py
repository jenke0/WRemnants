...
narf.clingutils.Declare('#include "muon_efficiencies_binned_vqt_integrated.hpp"')

data_dir = common.data_dir


def make_muon_efficiency_helpers_binned_vqt_integrated(
    filename=data_dir + "/muonSF/allSmooth_GtoH.root",
    filenamevqt=data_dir + "/muonSF/allSmooth_GtoH.root",
    filenamevqttriggerplus="/gpfs/ddn/cms/user/bruschin/newfit3/egm_tnp_analysis/plots/triggerefficiencies17012023/efficiencies_GtoH/mu_trigger_plus/allEfficiencies_2D.root",
    filenamevqttriggerminus="/gpfs/ddn/cms/user/bruschin/newfit3/egm_tnp_analysis/plots/triggerefficiencies17012023/efficiencies_GtoH/mu_trigger_minus/allEfficiencies_2D.root",
    era=None,
    is_w_like=False,
    max_pt=np.inf,
    ...
    eradict = {"2016PreVFP": "BtoF", "2016PostVFP": "GtoH"}
    eratag = eradict[era]
    ...
    # finvqt = ROOT.TFile.Open(filenamevqt)
    # if finvqt is None or finvqt.IsZombie():
    #    print(f"Error: file {filenamevqt} was not opened correctly")
    #    quit()

    histNameTag = "smoothWithOriginalPtBins" if usePseudoSmoothing else "original"
    histAltNameTag = "originalDataAltSig"

    nomiAltTypes = {0: histNameTag, 1: histAltNameTag}

    ## because of different pt binning cannot put all histograms in the same boost histogram
    effSyst_manager = {
        "sf_reco": {"axisLabels": ["reco"], "boostHist": None},
        "sf_tracking": {"axisLabels": ["tracking"], "boostHist": None},
        "sf_other": {
            "axisLabels": [
                "idip",
                "trigger",
    ...
    for effSystKey in effSyst_manager.keys():
        ...
        histPrefix = (
            "effData"
            if "effData" in effSystKey
            else "effMC" if "effMC" in effSystKey else "SF"
        )
        for charge, charge_tag in charges.items():
            for eff_type in effSyst_manager[effSystKey]["axisLabels"]:
                for nomiAltId, nomiAltTag in nomiAltTypes.items():
                    ...
                    chargeTag = (
                        charge_tag if eff_type in chargeDependentSteps else "both"
                    )
                    hist_name = (
                        f"{histPrefix}_{nomiAltTag}_{eratag}_{eff_type}_{chargeTag}"
                    )
                    ...
                    hist_hist = narf.root_to_hist(
                        hist_root, axis_names=["SF eta", "SF pt"]
                    )
                    ...
                    if effSyst_manager[effSystKey]["boostHist"] is None:
                        ...
                        axis_eff_type = hist.axis.StrCategory(
                            effSyst_manager[effSystKey]["axisLabels"],
                            name=f"{effSystKey}_eff_type",
                        )
                        effSyst_manager[effSystKey]["boostHist"] = hist.Hist(
                            axis_eta_eff,
                            axis_pt_eff,
                            axis_charge,
                            axis_eff_type,
                    ...
                    effSyst_manager[effSystKey]["boostHist"].view(flow=True)[
                        :,
                        :,
                        axis_charge.index(charge),
                        axis_eff_type.index(eff_type),
                        nomiAltId,
                    ] = hist_hist.view(flow=True)[:, :]

        # set overflow and underflow eta-pt bins equal to adjacent bins
        effSyst_manager[effSystKey]["boostHist"].view(flow=True)[0, ...] = (
            effSyst_manager[effSystKey]["boostHist"].view(flow=True)[1, ...]
        )
        effSyst_manager[effSystKey]["boostHist"].view(flow=True)[
    ...
    sf_tracking_pyroot = narf.hist_to_pyroot_boost(
        effSyst_manager["sf_tracking"]["boostHist"]
    )
    sf_other_pyroot = narf.hist_to_pyroot_boost(
        effSyst_manager["sf_other"]["boostHist"]
    )

    vqtstring = ROOT.std.string(filenamevqt)

    if includeTrigger:
        vqtstringplus = ROOT.std.string(filenamevqttriggerplus)
        vqtstringminus = ROOT.std.string(filenamevqttriggerminus)
        helper = ROOT.wrem_vqt_int.muon_efficiency_binned_helper[
            str(is_w_like).lower(),
    ...
    #                                                      ROOT.std.move(sf_reco_pyroot),
    #                                                  )
    helper_syst = ROOT.wrem_vqt_int.muon_efficiency_binned_helper_syst[
        str(is_w_like).lower(),
        type(sf_other_pyroot),
        type(sf_tracking_pyroot),
        type(sf_reco_pyroot),
    ...
    for effStatKey in effStat_manager.keys():
        ...
        for charge, charge_tag in charges.items():
            for eff_type in effStat_manager[effStatKey]["axisLabels"]:
                ...
                effStat_manager[effStatKey]["boostHist"].view(flow=True)[
                    :, :, axis_charge.index(charge), axis_eff_type.index(eff_type)
                ] = hist_hist.view(flow=True)[:, :]
        ...
        nptbins = (
            np.count_nonzero(axis_pt_eff.edges < max_pt)
            if "tracking" not in effStatKey
            else axis_pt_eff.size
        )
        ...
        if "sf_iso" in effStatKey:
            helper_stat = ROOT.wrem.muon_efficiency_binned_helper_stat_iso[
                str(is_w_like).lower(), netabins, nptbins, type(sf_stat_pyroot)
            ](ROOT.std.move(sf_stat_pyroot))
        else:
            helper_stat = ROOT.wrem.muon_efficiency_binned_helper_stat[
                str(is_w_like).lower(), netabins, nptbins, type(sf_stat_pyroot)
            ](ROOT.std.move(sf_stat_pyroot))
        ...
        if isinstance(axis_pt_eff, bh.axis.Regular):
            axis_pt_eff_tensor = hist.axis.Regular(
                axis_pt_eff.size,
                axis_pt_eff.edges[0],
                axis_pt_eff.edges[-1],
                name="nPtBins",
            ...
            axis_pt_eff_tensor = hist.axis.Variable(
                axis_pt_eff.edges[: nptbins + 1],
                name="nPtBins",
                overflow=False,
                underflow=False,
            )
