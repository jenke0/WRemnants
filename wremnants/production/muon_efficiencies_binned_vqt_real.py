...
def make_muon_efficiency_helpers_binned_vqt_real(
    ...
    filenames = ROOT.std.vector(
        "string"
    )()  # order is isolation, triggerminus, triggerplus
    ...
    triggerminus3dfilename = f"{data_dir}/muonSF/triggerminus3DSFUT.root"
    triggerminus3dhistoname = "SF3D_nominal_trigger_minus"
    triggerplus3dfilename = f"{data_dir}/muonSF/triggerplus3DSFUT.root"
    ...
    helper = ROOT.wrem_vqt_real.muon_efficiency_binned_helper[
        str(is_w_like).lower(),
        ...
        for ic, (charge, charge_tag) in enumerate(charges.items()):
            for eff_type in effStat_manager[effStatKey]["axisLabels"]:
                ...
                effStat_manager[effStatKey]["boostHist"].view(flow=True)[
                    :, :, axis_charge_def.index(charge), axis_eff_type.index(eff_type)
                ] = hist_hist.view(flow=True)[:, :]
