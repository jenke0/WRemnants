...
def makehelicityWeightHelper_polvar(
    genVcharge=-1, fileTag="x0p40_y3p50_V6", filePath=".", noUL=False
):
    ...
    charges = {-1.0: "minus", 1.0: "plus", 0.0: "Z"}
    ...
    filenames = {-1: filenamePlus, 1: filenameMinus, 0: filenameZ}

    chargeTag = f"genChargeV{charges[genVcharge]}"

    logger.debug(
        f"Preparing helicity weights for theory agnostic analysis: gen V charge {charges[genVcharge]}"
    )
    ...
    if noUL:
        ...
        axis_helicity_part = hist.axis.Integer(
            -1, len(folders), name="helicityPart", overflow=False, underflow=False
        )
    else:
        folders = ["UL"] + [f"A{i}" for i in range(uptorange)]
        axis_helicity_part = hist.axis.Integer(
            -1, len(folders) - 1, name="helicityPart", overflow=False, underflow=False
        )
    ...
    for ifld, fld in enumerate(folders):
        ...
        # start with nominal
        hnom = root_io.safeGetRootObject(f, f"h_pdf_{fld}")
        ...
        if noUL:
            ...
            nomIndex = ifld
        hnom_hist_full.values(flow=False)[:, :, nomIndex] = hnom_hist.values(
            flow=False
        )[:, :]
        # now the syst
        nSysts = 0
        for k in f.GetListOfKeys():
            name = k.GetName()
            if (
                name.startswith("h_pdf")
                and "syst" in name
                and not "muF" in name
                and not "muR" in name
                and name.endswith("Up")
            ):
                nSysts += 1
        # create and fill boost histogram
        if hsys_hist_full[fld] is None:
            axis_nsys = hist.axis.Integer(
                0, nSysts, name="nSyst", overflow=False, underflow=False
            )
            hsys_hist_full[fld] = hist.Hist(
                axis_qtOverQ,
                axis_absY,
                axis_nsys,
                down_up_axis,
        ...
        for isys in range(nSysts):
            hsysDown = root_io.safeGetRootObject(f, f"h_pdf_{fld}_syst{isys}Down")
            ...
            hsysUp = root_io.safeGetRootObject(f, f"h_pdf_{fld}_syst{isys}Up")
    ...
    hnom_hist_full.values(flow=True)[axis_qtOverQ.extent - 1, ...] = (
        hnom_hist_full.values(flow=True)[axis_qtOverQ.extent - 2, ...]
    )
    hnom_hist_full.values(flow=True)[:, 0, ...] = hnom_hist_full.values(flow=True)[
        :, 1, ...
    ]
    hnom_hist_full.values(flow=True)[:, axis_absY.extent - 1, ...] = (
        hnom_hist_full.values(flow=True)[:, axis_absY.extent - 2, ...]
    )
    ...
    for ifld, fld in enumerate(folders):
        ...
        # set overflow and underflow qt/Q-y bins equal to adjacent bins
        hsys_hist_full[fld].values(flow=True)[0, ...] = hsys_hist_full[fld].values(
            flow=True
        )[1, ...]
        hsys_hist_full[fld].values(flow=True)[axis_qtOverQ.extent - 1, ...] = (
            hsys_hist_full[fld].values(flow=True)[axis_qtOverQ.extent - 2, ...]
        )
        hsys_hist_full[fld].values(flow=True)[:, 0, ...] = hsys_hist_full[fld].values(
            flow=True
        )[:, 1, ...]
        hsys_hist_full[fld].values(flow=True)[:, axis_absY.extent - 1, ...] = (
            hsys_hist_full[fld].values(flow=True)[:, axis_absY.extent - 2, ...]
        )
        ...
        helper = ROOT.wrem.WeightByHelicityHelper_polvar[
            genVcharge,
            varIndex,
            nSysts,
            type(hsys_hist_full_pyroot),
            type(hnom_hist_full_pyroot),
        ](ROOT.std.move(hsys_hist_full_pyroot), ROOT.std.move(hnom_hist_full_pyroot))
        ## define axis for syst variations
        axis_nsyst = hist.axis.Integer(
            0, nSysts, name="nPolVarSyst", overflow=False, underflow=False
        )
